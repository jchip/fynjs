...
| [pnpm-benchmark-replication.md](pnpm-benchmark-replication.md) | Running pnpm's install benchmark locally with fyn added, at several link settings: where pnpm's 10x headline comes from, how pnpm gets its speed (concurrency, APFS clones), and what it means for fyn (socket-cap fix, serial resolve then fetch, central mode on macOS). Harness edits in [pnpm-benchmark-harness.patch](pnpm-benchmark-harness.patch) |
