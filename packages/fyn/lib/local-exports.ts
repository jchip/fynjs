...
export const resolveLocalExportsConfig = pkg => {
...
export const localExportsScanIgnores = pkg => {
  let config;
  try {
...
export async function makeLocalExportsManifest({ cwd, depInfos, config: exportsConfig }) {
...
export async function localExportsNeedInstall({ cwd, manifest }) {
...
export async function reconcileLocalExports({ cwd, manifest, previous }) {
  const desired = normalizeManifest(manifest);
  const byRoot = groupByRoot(desired.exports);
...
export async function syncLocalExports(options) {
