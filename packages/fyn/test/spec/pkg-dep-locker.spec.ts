...
describe("pkg-dep-locker", function () {
  ...
  it("should keep a pathful registry tarball URL unchanged", () => {
    ...
    expect(convertTarball("https://packages.idme.co/artifactory/api/npm/npm/", tarballUrl)).toBe(
      tarballUrl,
    );
  });
  ...
  it("should rewrite a pathful registry tarball URL without duplicating the registry path", () => {
    ...
    expect(convertTarball(currentRegistry, tarballUrl)).toBe(
      "https://packages.idme.co/artifactory/api/npm/npm/@anthropic-ai/sdk/-/sdk-0.52.0.tgz",
    );
  });
  ...
  describe("corrupt lock detection", function () {
    ...
    it("convert() accepts a self-consistent lock entry", () => {
      ...
      expect(Array.isArray(locked)).toBe(false);
      expect(locked.versions).toHaveProperty("10.49.0");
    });
    ...
    it("convert() expands the serialized form instead of handing it back", () => {
      ...
      expect(vpkg).not.toHaveProperty("$");
      expect(vpkg).not.toHaveProperty("_");
      expect(vpkg).not.toHaveProperty("top");
      expect(vpkg._hasShrinkwrap).toBe(true);
      expect(vpkg.dist).toStrictEqual({ integrity: undefined, tarball: "react.tgz" });
      expect(vpkg.fromLocked).toBe(true);
      expect(vpkg.name).toBe("@sentry/react");
      expect(vpkg.version).toBe("10.49.0");
      // the entry carried over from the lock, untouched
      expect(vpkg.dependencies).toStrictEqual({ "@sentry/core": "10.49.0" });
    });
    ...
    it("convert() leaves _hasShrinkwrap false when the lock does not set it", () => {
      ...
      expect(vpkg._hasShrinkwrap).toBe(false);
    });
    ...
    it("read() ignores a lockfile with git conflict markers", async () => {
      ...
      expect(locker._lockData).toStrictEqual({});
    });
  });
  ...
  describe("failed optional dependencies", function () {
    ...
    it("should not record a platform failure, which any reader re-derives from os/cpu", () => {
      ...
      expect(meta).not.toHaveProperty("optFailed");
      ...
      expect(meta.os).toStrictEqual(["linux"]);
      expect(meta.cpu).toStrictEqual(["x64"]);
      expect(meta.$).toBe("sha512-test");
      expect(meta._).toBe("https://registry.npmjs.org/mod-bad-os/-/mod-bad-os-1.0.0.tgz");
    });
    ...
    it("should not claim a platform failure has no package.json recorded", () => {
      expect(genLock(OPT_FAILED_PLATFORM)).not.toHaveProperty("_missingJson");
      expect(genLock(OPT_FAILED_PLATFORM)).not.toHaveProperty("dependencies");
    });
    ...
    it("should record the dependencies a platform failure's meta carried", () => {
      ...
      expect(meta.dependencies).toStrictEqual({ "mod-a": "^1.0.0" });
      expect(meta.peerDependencies).toStrictEqual({ "mod-b": "^2.0.0" });
      expect(meta).not.toHaveProperty("_missingJson");
    });
