...
describe("hard-link-dir", function () {
  ...
  it("links non-JS source maps (.d.ts.map/.css.map) in non-CI mode", async () => {
    ...
    try {
      ...
      expect(fs.existsSync(Path.join(dest, "dist/types.d.ts.map"))).toBe(true);
      expect(fs.existsSync(Path.join(dest, "dist/styles.css.map"))).toBe(true);
      // the .d.ts itself is linked too
      expect(fs.existsSync(Path.join(dest, "dist/types.d.ts"))).toBe(true);
    } finally {
...
describe("hard-link-dir - never writes through an existing hardlink (FPM-52)", function () {
  const withFixture = async (fn) => {
    ...
    try {
      ...
      expect(fs.statSync(target).ino).toBe(fs.statSync(other).ino);
  ...
  it("cloneFile must not corrupt the linked sibling", async () => {
    ...
    expect(r.target).toBe("js-launcher-from-tarball");
    // the whole point: the other package's file is untouched
    expect(r.other).toBe("native-binary");
    expect(r.stillLinked).toBe(false);
  });
