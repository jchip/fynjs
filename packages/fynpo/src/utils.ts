...
export function makePublishFilter(
  ...
  // Managed is optional so callers constructing the historical basic shape remain eligible.
): (pkgInfo: PackageBasicInfo & { managed?: boolean }) => boolean {
  ...
  return (pkgInfo: PackageBasicInfo & { managed?: boolean }): boolean => {
    if (!pkgInfo || pkgInfo.managed === false) {
      return false;
    }
