...
describe("visual-logger", () => {
  ...
  describe("visual item", () => {
    ...
    it("should reset item spin index to 0 in addItem", async () => {
      ...
      await delay(100);
      //
      // Every render from addItem on has to show T back at frame 0, whichever render it
      // is - addItem triggers none of its own, so the first one carrying R is either the
      // tick after it or a render the previous tick scheduled and left pending, and which
      // of the two it is turns on how close to a tick the sleep above lands.
      //
      const withR = visList.filter(x => x.includes("R:"));
      expect(withR.length).toBeGreaterThan(0);
      expect(withR[0]).toEqual("TEST_1: \na T: foo\nx R: ");
    });
