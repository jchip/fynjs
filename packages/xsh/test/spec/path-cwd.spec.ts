...
describe("path-cwd", function () {
  it("should leave path w/o cwd as is", () => {
    expect(xsh.pathCwd.remove("/test/foo/blah")).toBe("/test/foo/blah");
  });

  it("remove cwd from path", () => {
    expect(xsh.pathCwd.remove(Path.resolve("foo", "bar"))).toBe(Path.normalize("/foo/bar"));
  });
  ...
  it("remove all cwd from path with g flag", () => {
    ...
    expect(xsh.pathCwd.remove(`${x} ${x}`, "g")).toBe(`${e} ${e}`);
  });

  it("replace cwd from path", () => {
    expect(xsh.pathCwd.replace(Path.resolve("foo", "bar"))).toBe(Path.normalize("CWD/foo/bar"));
    expect(xsh.pathCwd.replace(Path.resolve("foo", "bar"), "$")).toBe(
  ...
  it("replace all cwd from path with g flag", () => {
    ...
    expect(xsh.pathCwd.replace(`${x} ${x}`, false, "g")).toBe(`${e1} ${e1}`);
    expect(xsh.pathCwd.replace(`${x} ${x}`, "$", "g")).toBe(`${e2} ${e2}`);
  });

  it("remove should strip leading /", () => {
    expect(xsh.pathCwd.remove(Path.resolve("foo"), "", true)).toBe("foo");
  });
  ...
  describe("cwd with regex metacharacters", function () {
    ...
    it("removes a cwd containing parens", () => {
      ...
      withCwd(cwd, () => {
        expect(xsh.pathCwd.remove(Path.join(cwd, "lib", "a.js"))).toBe(
    ...
    it("removes a cwd containing a plus", () => {
      ...
      withCwd(cwd, () => {
        expect(xsh.pathCwd.remove(Path.join(cwd, "x"))).toBe(Path.normalize("/x"));
      });
    });
    ...
    it("does not let a dot in cwd match any character", () => {
      ...
      withCwd(cwd, () => {
        ...
        expect(xsh.pathCwd.replace(other)).toBe(other);
        expect(xsh.pathCwd.replace(Path.join(cwd, "lib", "a.js"))).toBe(
