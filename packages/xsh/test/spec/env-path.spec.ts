...
describe("envPath", function () {
  ...
  it("should add to a custom env/path", () => {
    ...
    expect(path).toBe(["test", "blah", "foo"].join(Path.delimiter));
    const env = {};
    path = xsh.envPath.add("foo", env);
    expect(path).toBe("foo");
    expect(env).toStrictEqual({ [pathKey]: "foo" });
    path = xsh.envPath.add("", env);
    expect((env as any)[pathKey]).toBe("foo");
  });

  it("addToFront should add path to front", () => {
    xsh.envPath.addToFront("/test1");
    expect(process.env[pathKey]!.indexOf("/test1")).toBe(0);
    delete process.env[pathKey];
    xsh.envPath.addToFront("/test1");
    xsh.envPath.addToFront("/test1");
    expect(process.env[pathKey]).toBe("/test1");
    xsh.envPath.addToFront("/test2");
    expect(process.env[pathKey]!.indexOf("/test2")).toBe(0);
    xsh.envPath.addToFront("/test1");
    expect(process.env[pathKey]!.indexOf("/test1")).toBe(0);
  });
  ...
  it("addToFront should add path to front of a path", () => {
    ...
    expect(PATH.indexOf("/test1")).toBe(0);
    PATH = xsh.envPath.addToFront("/test1", {});
    PATH = xsh.envPath.addToFront("/test1", PATH);
    expect(PATH).toBe("/test1");
    PATH = xsh.envPath.addToFront("/test2", PATH);
    expect(PATH.indexOf("/test2")).toBe(0);
    PATH = xsh.envPath.addToFront("/test1", PATH);
    expect(PATH.indexOf("/test1")).toBe(0);
  });
  ...
  it("addToFront should remove duplicates from anywhere in PATH", () => {
    ...
    expect(PATH).toBe([p2, p1, p3].join(Path.delimiter));
    expect(PATH.split(Path.delimiter).filter(x => x === p2).length).toBe(1);

    // Test: duplicate at end of PATH
    PATH = [p1, p2, p3].join(Path.delimiter);
    PATH = xsh.envPath.addToFront(p3, PATH);
    expect(PATH).toBe([p3, p1, p2].join(Path.delimiter));
    expect(PATH.split(Path.delimiter).filter(x => x === p3).length).toBe(1);

    // Test: already at front - should not create duplicates
    PATH = [p1, p2, p3].join(Path.delimiter);
    PATH = xsh.envPath.addToFront(p1, PATH);
    expect(PATH).toBe([p1, p2, p3].join(Path.delimiter));
    expect(PATH.split(Path.delimiter).filter(x => x === p1).length).toBe(1);
    ...
    expect(PATH).toBe([p4, p1, p2, p3].join(Path.delimiter));
  });
  ...
  it("addToFront should handle empty string and non-string inputs", () => {
    ...
    expect(PATH).toBe([p1, p2].join(Path.delimiter));
  ...
  it("add should add path end if it's not exist", () => {
    ...
    xsh.envPath.add(p1);
    expect(process.env[pathKey]).toBe(r1);
    xsh.envPath.add(p2);
    expect(process.env[pathKey]).toBe(r1);
    xsh.envPath.add(p3);
    expect(process.env[pathKey]).toBe(r2);
  });

  it("findEnvKey should find correct path env key", () => {
    const defaultKey = xsh.envPath.findEnvKey(process.env);
    expect(defaultKey!.toLowerCase()).toBe("path");
    expect(defaultKey).toBe(xsh.envPath.envKey);
  });
});
