...
/**
 * The built-in compose options.
 *
 * The provider names are themselves templates, resolved against `context`
 * before any file is looked for - which is how `{{deployment}}` becomes
 * `production.json` and `{{hostname}}-{{instance}}` drops out entirely when
 * neither is set.
 *
 * @returns a fresh copy of the defaults, safe for the caller to merge into
 */
export function defaultOpts() {
  return {
    dir: "config",
    extSearch: ["json", "yaml", "js", "ts"],
    // a copy, not the shared module object - callers merge their own handlers
    // over these options, and lodash merges in place
