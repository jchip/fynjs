...
function getArg({ opt, msg, argv, value = true, valids = [] }) {
    ...
    if (tagIx >= 0) {
        if (value) {
            ...
            if (valids.length > 0) {
                assert.ok(valids.includes(val), `${opt} has a value ${val} but it must be one of: ${valids.join(", ")}`);
            }
            else {
...
export async function npmPublish({ exit = true, silent = false, } = {}) {
    const noop = () => { };
    ...
    try {
        ...
        if (scripts.prepublish) {
            console.error("Warning: You have 'prepublish' npm script which is deprecated - npm publish will not run it.");
